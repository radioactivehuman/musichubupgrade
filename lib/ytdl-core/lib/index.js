...
//     // returnDownloadUrl(info,options);
...
const ytdl = async (link, options) => {
  ...
  try {
    ...
    const mainUrl = await returnDownloadUrl(info, options);
    // console.log('the main url is', mainUrl);
    return mainUrl;
  } catch (error) {
  ...
  // ytdl.getInfo(link, options).then(info => {
  //   // downloadFromInfoCallback(stream, info, options);
  //   theMainUrl = returnDownloadUrl(info, options)
  //   //   console.log(returnDownloadUrl(info,options))
  //   //  return returnDownloadUrl(info,options);
  //   console.log('the main urll is', theMainUrl)
  //   return `here is your url ${theMainUrl.length}`;

  // },
  // );

};
// const ytdlRiv = (link, options) => {
//   const stream = createStream(options);
//   ytdl.getInfo(link, options).then(info => {
//     returnDownloadUrl(info,options);
//     downloadFromInfoCallback(stream, info, options);
...
const downloadFromInfoCallback = (stream, info, options) => {
  ...
  // returnDownloadUrl(info,options);
  // console.log('the info is', info.formats);
  // console.log('the options are', options);

  // console.log('Download URL1:', format.url)
...
const returnDownloadUrl = (info, options) => {
  options = options || {};
  try {
    let format;
    try {
    ...
    // console.log('Download URL69:', format.url)
    return format.url;
    // console.log('Download URL69:', format.url)
  } catch (error) {
...
module.exports.returnDownloadUrl = returnDownloadUrl;